...
class DeepSpeech(pl.LightningModule):
    def __init__(self,
        ...
        self.conv = MaskConv(nn.Sequential(
            ...
            # # 新增层1
            # nn.Conv2d(32, 32, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2)),  # 增大感受野并保持大小不变
            # nn.BatchNorm2d(32),
            # nn.Hardtanh(0, 20, inplace=True),
            # # 新增层2
            # nn.Conv2d(32, 32, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2)),  # 增大感受野并保持大小不变
        ...
        self.down1 = DoubleConv(32, 32)
        self.down2 = DoubleConv(32, 32)
        #self.down3 = DoubleConv(32, 32)

        # self.up1 = Up(192,64,True)
        # self.up2 = Up(96,32,True)
        
        self.myjpu1 = JPU([32, 32, 32])
        #self.myjpu2 = JPU([32, 32, 32])
    ...
    def forward(self, x, lengths, hs=None):
        ...
        x2 = self.down2(x1)
        #x3 = self.down2(x2)        
        _, _, _, x = self.myjpu1(x, x1 ,x2)
        #_, _, _, x = self.myjpu1(x ,x, x)        
        # x = self.up1(x2, x1)
        # x = self.up2(x, x_j1)
...
class JPU(nn.Module):
    def __init__(self, in_channels, width=32, norm_layer=None, up_kwargs=None):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels[-2], width, 5, padding=2, bias=False),
            # norm_layer(width),
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))
        #64
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels[-3], width, 5, padding=2, bias=False),
            # norm_layer(width),
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True))
